...
def get_acc(dataloader, model):
    ...
    for xx_pad, targets, x_lens, _ in dataloader:
        ...
        for i in range(preds.shape[0]):
            ...
            char_matches = sum(pred.argmax(1) == target.argmax(1))
            char_acc = (np.divide(char_matches, pred.shape[0]) * 100).detach().numpy().item()
            char_accs.append(char_acc)
    char_acc = np.mean(char_accs)
    word_acc = np.mean([a == 100. for a in char_accs]) * 100.
    result_dict = {"wa": word_acc, "ca": char_acc}
    result_str = f"wa: {word_acc:.3f}%, ca: {char_acc:.3f}%"
